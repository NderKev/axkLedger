...
const polygon_testnet_rpc = `https://polygon-amoy.infura.io/v3/${process.env.INFURA_PROJECT_ID}`;
//const {BigNumber} = require("bignumber-js");
//const provider = new Theta.providers.HttpProvider('https://eth-rpc-api-testnet.thetatoken.org/rpc');
const web3 = new Web3(polygon_testnet_rpc);
...
const contractAddress = '0x46b6524a00d774eca8c1b93d3631ac41b8f00b94';
...
async function transferFrom(from, to, value, fromAddress, privateKey) {
    const _value = value * ten18;//(new BigNumber(value)).multipliedBy(ten18);
...
async function allowance(owner, spender) {
    const allowance = await AXKContract.allowance(owner, spender);
    return allowance;
}

// Function to get user address Axk Token balance for an account address 
async function balanceOf(account) {
    const balance = await AXKContract.balanceOf(account);
    return balance;
}

// Function to get Axk Token cap
async function cap() {
    const cap = await AXKContract.cap();
    return cap;
}

// Function to get Axk Token decimals
async function decimals() {
    const decimals = await AXKContract.decimals();
    return decimals;
}

// Function to get Axk Token name
async function name() {
    const name = await AXKContract.name();
    return name;
}

// Function to get Axk Token nonces
async function nonces(ownerAddress) {
    const nonces = await AXKContract.nonces(ownerAddress);
    return nonces;
}

// Function to get Axk Token owner
async function owner() {
    const owner = await AXKContract.owner();
    return owner;
}

// Function to check if Axk Token is paused
async function paused() {
    const paused = await AXKContract.paused();
    return paused;
}

// Function to get Axk Token symbol
async function symbol() {
    const symbol = await AXKContract.symbol();
    return symbol;
}

// Function to get Axk Token total supply
async function totalSupply() {
    const totalSupply = await AXKContract.totalSupply();
    return totalSupply;
}
