...
const {abi} = require("../abi/ProduceTraceabilityV1.json");
const ProduceManagement = require("./ProduceManagement");
const contractAddress = contracts.ProduceTraceabilityV1;

const ProduceTraceabilityV1Contract = new web3.eth.Contract(abi, contractAddress);
...
async function registerFarmer(data) { 
    const tx = ProduceTraceabilityV1Contract.methods.registerFarmer(data.name, data.location, data.address);
...
async function verifyFarmer(data) { 
    const tx = ProduceTraceabilityV1Contract.methods.verifyFarmer(data);
...
async function addFarmProduce(data) { 
    ...
    const tx = ProduceTraceabilityV1Contract.methods.addFarmProduce(data.produce, data.lot_number, data.weight, data.quantity, data.storage_date, data.farmer, data.agents);
...
async function sellFarmProduce(data) { 
    data.index = Math.floor(Math.random() * 9000000000) + 1000000000;
    const tx = ProduceTraceabilityV1Contract.methods.sellFarmProduce(data.hash, data.index, data.farmer, data.buyer, data.amount, data.price);
...
async function getFarmer(data) {
    const farmer = await ProduceTraceabilityV1Contract.methods.getFarmer(data).call();
    return farmer;
}

// Function to get farmer produce details
async function getProduce(data) {
    const produce = await ProduceTraceabilityV1Contract.methods.getProduce(data).call();
    return produce;
}

// Function to get farmer produce index
async function getProduceIndex(data) {
    const index = await ProduceTraceabilityV1Contract.methods.getProduceIndex(data).call();
    return index;
}

// Function to get farmer produce sale
async function getProduceSale(data) {
    const prod_sale = await ProduceTraceabilityV1Contract.methods.getProduceSale(data).call();
    return prod_sale;
}

// Function to get farmer produce sale index
async function getProduceSaleIndex(data) {
    const sale = await ProduceTraceabilityV1Contract.methods.getProduceSaleIndex(data).call();
    return sale;
}

// Function to get farmer produce sale index
async function isProduce(data) {
    const hash = await ProduceTraceabilityV1Contract.methods.isProduce(data).call();
    return hash;
}
